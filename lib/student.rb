...
DB[:conn].execute(sql).map {|s| Student.new_from_db(s)}
  ...
  def self.all_students_in_grade_9
    ...
    query_result = DB[:conn].execute(sql).map do |student|
  ...
  def self.first_X_students_in_grade_10(x)
    ...
    query_result = DB[:conn].execute(sql, x).map {|s| Student.new_from_db(s)}
    

  end

  def self.first_student_in_grade_10
    Student.first_X_students_in_grade_10(10000000000).first
  end
